...
module.exports = async function (req, res, next) {
  try {
    ...
    if (session) {
      const authSession = await AuthSession.findOne({ token: session });
      if (!authSession) {
        throw new RequestIsBastardError(
          "Your session is not valid, please login"
        );
      }
    }else{
      throw new RequestIsBastardError(
    ...
    if (user) {
      const authenticatedUser = await User.findById(user);
      if (!authenticatedUser) {
        throw new RequestIsBastardError(
          "Invalid user header. You are not allowed to access this route. Please contact to istiaq.me@gmail.com."
        );
      }
    }
  else{
